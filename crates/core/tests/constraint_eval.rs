...
fn reg() -> Registry { Registry::with_builtins() }
...
fn always_is_true() {
    ...
    let ev = event_at(0);
    assert!(ConstraintExpr::Always.evaluate(&ctx(&ev, &store), &reg()).unwrap());
}
...
fn not_inverts_always() {
    let store = InMemoryStateStore::new();
    let ev = event_at(0);
    let expr = ConstraintExpr::Not { expr: Box::new(ConstraintExpr::Always) };
...
fn not_not_always_is_true() {
    ...
    let expr = ConstraintExpr::Not {
        expr: Box::new(ConstraintExpr::Not { expr: Box::new(ConstraintExpr::Always) }),
    };
    assert!(expr.evaluate(&ctx(&ev, &store), &reg()).unwrap());
}
...
fn all_empty_is_true() {
    let store = InMemoryStateStore::new();
    let ev = event_at(0);
    assert!(ConstraintExpr::All { exprs: vec![] }.evaluate(&ctx(&ev, &store), &reg()).unwrap());
}
...
fn all_short_circuits_on_first_false() {
    let store = InMemoryStateStore::new();
    let ev = event_at(0);
    let not_always = ConstraintExpr::Not { expr: Box::new(ConstraintExpr::Always) };
    let expr = ConstraintExpr::All {
...
fn any_empty_is_false() {
    let store = InMemoryStateStore::new();
    let ev = event_at(0);
    assert!(!ConstraintExpr::Any { exprs: vec![] }.evaluate(&ctx(&ev, &store), &reg()).unwrap());
}
...
fn any_all_false_is_false() {
    ...
    let ev = event_at(0);
    let not_always = || ConstraintExpr::Not { expr: Box::new(ConstraintExpr::Always) };
    let expr = ConstraintExpr::Any { exprs: vec![not_always(), not_always()] };
    assert!(!expr.evaluate(&ctx(&ev, &store), &reg()).unwrap());
}
...
fn de_morgan_not_all_equals_any_not() {
    ...
    let not_a_and_b = ConstraintExpr::Not { expr: Box::new(a_and_b) };
...
fn var_leaf(key: &str, op: CompareOp, val: Value) -> ConstraintExpr {
    ConstraintExpr::Leaf {
        constraint: ConstraintConfig::VarCompare { key: key.into(), op, value: val },
    }
}
...
fn var_compare_eq() {
    ...
    let ev = event_at(0);
    assert!(var_leaf("x", CompareOp::Eq, Value::Int(7)).evaluate(&ctx(&ev, &store), &reg()).unwrap());
    assert!(!var_leaf("x", CompareOp::Eq, Value::Int(8)).evaluate(&ctx(&ev, &store), &reg()).unwrap());
}
...
fn var_compare_gt() {
    ...
    let ev = event_at(0);
    assert!(var_leaf("counter", CompareOp::Gt, Value::Int(3)).evaluate(&ctx(&ev, &store), &reg()).unwrap());
    assert!(!var_leaf("counter", CompareOp::Gt, Value::Int(5)).evaluate(&ctx(&ev, &store), &reg()).unwrap());
    assert!(!var_leaf("counter", CompareOp::Gt, Value::Int(10)).evaluate(&ctx(&ev, &store), &reg()).unwrap());
}
...
fn var_compare_missing_key_is_null() {
    ...
    assert!(var_leaf("missing", CompareOp::Eq, Value::Null).evaluate(&ctx(&ev, &store), &reg()).unwrap());
    assert!(!var_leaf("missing", CompareOp::Eq, Value::Int(0)).evaluate(&ctx(&ev, &store), &reg()).unwrap());
}
...
fn time_leaf(from: &str, to: &str) -> ConstraintExpr {
    ConstraintExpr::Leaf {
        constraint: ConstraintConfig::TimeRange { from: from.into(), to: to.into() },
    }
}
...
fn time_range_in_range() {
    ...
    let store = InMemoryStateStore::new();
    let r = reg();
    assert!(time_leaf("00:00", "01:00").evaluate(&ctx(&ev, &store), &r).unwrap());
    assert!(time_leaf("00:30", "00:30").evaluate(&ctx(&ev, &store), &r).unwrap()); // exact boundary
}
...
fn time_range_out_of_range() {
    ...
    let store = InMemoryStateStore::new();
    let r = reg();
    assert!(!time_leaf("01:00", "02:00").evaluate(&ctx(&ev, &store), &r).unwrap());
    assert!(!time_leaf("00:31", "23:59").evaluate(&ctx(&ev, &store), &r).unwrap());
}
...
fn time_range_wraps_midnight() {
    ...
    assert!(time_leaf("23:00", "01:00").evaluate(&ctx(&ev, &store), &r).unwrap());
    // 09:00–10:00 should not match 23:30
    assert!(!time_leaf("09:00", "10:00").evaluate(&ctx(&ev, &store), &r).unwrap());
}
