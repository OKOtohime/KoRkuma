# `koakuma-actions` — 内置 Action 实现

> **Cargo 包名**: `koakuma-actions` · **路径**: `crates/actions/`
> **最后同步**: 2026-06-03 (M2.1：Action 异步化)

## 职责概述

`koakuma-actions` 提供管道末端的执行能力（Action 层）。与 `koakuma-core/builtins` 中轻量级的 `SetVariable`、`Delay` 互补，本 crate 实现较重量级或平台相关的 Action：进程执行、桌面通知、输入模拟。
...
| Crate | 用途 |
|-------|------|
| `koakuma-core` | `Action` trait、`ActionConfig`、`ExecContext`、权限类型 |
...
| `SetVariable` | ✅ M1.1 | 在 `koakuma-core/builtins` |
| `Delay` | ✅ M1.1 | 在 `koakuma-core/builtins` |
